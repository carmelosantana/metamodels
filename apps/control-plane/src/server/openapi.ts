...
const COMPONENT_RESPONSES: Record<string, ResponseObject> = {
  ...
  KeySetUnavailable: errorResponse(
    'The OpenID Provider\'s key set could not be fetched, so the token was never judged. 503 ' +
      'rather than 401 on purpose: the fault is the server\'s, and a 401 would send the client off ' +
      'to refresh a token that is probably fine.',
    {
      headers: {
        'Retry-After': {
          description:
            'Seconds. Matches the JWKS fetch cooldown, so every retry inside the window would be ' +
            'answered from the same cached failure.',
          schema: { type: 'integer' },
        },
      },
    },
