...
pinned**; and `client_id` = `CLI_CLIENT_ID`, the admin CLI, which is the only client the OP issues an
admin-API token to (§8, A15).
...
- The negative cases: no token → 401; bearer **and** cookie → 400; cookie only → 401; a token whose
  `aud` is not `adminApiResource(CONSOLE_URL)` → 401; a token issued to another `client_id` → 401;
  an unscoped token → 403 naming the capability;
...
where the row records the shape the implementation gave it. A1, A11 and A15 are also corrected in place
(§4.4; D10, §2.6 and §4.5; §3.2), as are two passages that were wrong rather than changed: §2.2's
...
| A3 | 4.4 | "Gate on the third argument" | `makeGetResourceServerInfo(servers, allowedByClient)`: a resource must be declared *and* listed for the requesting client, or `invalid_target`. A client not in `resourcesByClient` gets no resource. ~~The console keeps its M1 admin-API access~~ **Superseded by A15:** the console is no longer listed and gets `invalid_target` for the admin API | The gate runs on every mint path (authorization, device authorization, token, refresh), so a new client is refused by default rather than trusted by default | Spec, as built |
...
| A5 | 4.4 | — | `verification_uri_complete` opens **our** code-entry page with the code prefilled and a visible Continue button, under the unchanged strict CSP. Two route intercepts replace oidc-provider pages that rely on inline script: the prefill on `GET /device` (`device-middleware.ts`), and the account switch (`switchAccountMiddleware` in `switch-account.ts`), which covers `device_resume` and, since PR #20, the console's `resume` route too | Those library pages render blank under `default-src 'none'`. The intercepts depend on oidc-provider internals, pinned to `~9.12.2` and by tests | Owner (prefill); the switch page follows from A4 |
...
| A10 | 1 | — | **`GET /flocks` and `GET /flocks/{id}` return `upstreamAuth` in plain text** to any token with `read`. Shipped documented ([Routes](../../admin-api.md#routes)); the fix — sealed at rest, never returned — is its own milestone on branch `feat/encrypt-upstream-auth`. **Resolved there:** `upstreamAuth` is write-only, responses carry `hasUpstreamAuth`, and a `PUT` that omits it keeps the stored credential ([sealed upstream credentials](2026-09-22-sealed-upstream-auth-design.md)) | Encrypting it needs a migration, a console change and a key-management decision: not an M2 rider | Owner |
...
| A12 | 3 | — | **Every successful admin-API mutation publishes the data plane's config invalidation**, with the same reason the console action for the same service call passes. `withAdmin(handler, { invalidates })` publishes after a 2xx response and never on a refusal; 13 route handlers set it, one for each method and path that changes something; no `GET` sets it, nor does `DELETE /keys/{id}`, which always answers `405`. `publishConfigInvalidation` never throws, so a failed publish cannot turn a committed change into an error, as in the console | Without it, a change made through the admin API reached the data plane only when its config cache expired, while the console publishes an invalidation for the same change (whole-branch review I1) | Controller |
| A13 | 3.1 | `Link: <…>; rel="next"`, target unspecified | **The `Link` target is path-relative** (`</api/admin/v1/flocks?limit=50&cursor=…>`), the same form as the POST routes' `Location`. `mm` resolves it against the request URL (RFC 8288 §3.1). See [Pagination](../../admin-api.md#pagination) | In a Next route handler `req.url` is the container's own address (`http://localhost:3000`), not the public host, so an absolute target sent a client, and its bearer token, to its own localhost over plain http (whole-branch review I2) | Controller |
| A14 | 4.4 | — | **Operator procedures that end CLI sign-ins.** [Forcing everyone to sign in again](../../DEPLOY.md#forcing-everyone-to-sign-in-again) now deletes every `RefreshToken` and `Grant` row. [Cutting off a lost machine](../../admin-api.md#cutting-off-a-lost-machine) ends one user's sign-ins: deactivate, delete that user's `Session`, `RefreshToken` and `Grant` rows, then reactivate, after either waiting out the 12-hour console cookie or rotating `SESSION_SECRET`. `apps/auth/test/cli-sign-out.test.ts` reads both of those `DELETE` statements out of the docs and runs them as printed | Rotating `SESSION_SECRET` and `OIDC_COOKIE_KEYS` ends only browser sessions: a CLI sign-in is a refresh chain in the sign-in service's database. A refresh refused because the user is deactivated does not use up the token, so without the per-user delete, reactivating the user would revive a lost machine's chain | Controller |
| A15 | 3.2, 4.4 | A3: the console keeps its M1 admin-API access; `verifyAdminToken` checks `typ`, `iss`, `aud`, `exp` and `sub`, never the client | **Only the admin CLI (`metamodels-cli`) may hold an admin-API token**, locked twice. (1) `resourcesByClient` lists the CLI alone, so the console, like any unlisted client, gets `invalid_target` for the admin resource at every mint path; it signs operators in with `openid` only. (2) `verifyAdminToken` refuses a token whose `client_id` is not `CLI_CLIENT_ID` (from `@metamodels/schema`; there is no env for it) with a `TokenError`, answered with the same fixed 401 detail as every other rejected token, never the reason. See [Authentication](../../admin-api.md#authentication) | Review finding G1. The console's authorization-code flow auto-consents on a live OP session with no fresh password (A16), and A4's password-every-time rule covers only the device flow, so `CONSOLE_CLIENT_SECRET` plus an authorization request the attacker starts with their own PKCE verifier, which the victim's live OP session auto-consents to, minted a `user.manage` token with no password. A leaked code alone is useless, because PKCE is required for every client. The verifier's check keeps an M4 client that is listed in `resourcesByClient` by mistake from silently gaining admin power. The fixed 401 is so the refusal is no enumeration oracle | Owner |
| A16 | 4.4 | — (the parent spec: first-party consent is automatic) | **Auto-consent is for the console and the CLI** (`firstPartyClientIds` in `provider.ts`). Any other client reaching the consent prompt gets `access_denied` until M4's consent screen. For the console, consent is silent whenever the browser holds a live OP session, with no password asked; for the CLI, A4 still asks for the password on every device approval. Auto-consent grants scopes, not resources: which client may get an admin-API token is A15 | Recorded because the console's silent consent is the premise of A15. Unchanged by this milestone, apart from adding the CLI (plan Task 7) | Spec, as built |
