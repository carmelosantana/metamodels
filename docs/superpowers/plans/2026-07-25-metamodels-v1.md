...
### Carry-forward from Plan 6c whole-branch review (honor in later work)

Plan 6c shipped (branch `feat/metamodels-plan6c`, ff-merged to `main` at `c70b1ce`; plan-doc `6ccaa79`): **the Lemon Squeezy re-validation scheduler** — the last v1-acceptance item. A background loop periodically re-validates every org's entitlement so a license can lapse (or a remote change land) **without waiting for an operator to log in**; the existing 7-day offline grace absorbs the gap between passes. **Structural revision (decided with Carmelo, revising the earlier "own `apps/scheduler`" note):** it is a control-plane **`bin/scheduler.ts` entrypoint + a compose `scheduler` service**, NOT a standalone app — the license modules (`license-service`/`entitlement-service`/`ls-client`/`license-crypto`) are coupled to control-plane `authorize`/`audit` and compile only under control-plane's Next-flavored tsconfig (`noEmit`, non-`composite`, Bundler resolution), which a standalone tsx app cannot cleanly import under `tsc -b`. `bin/scheduler.ts` sits beside `bin/seed.ts` (proven to import `../src/server/*`). Pieces: `listEntitledOrgIds(db)` (cross-org enumerator, no actor / not capability-gated — reachable only server-internally); `revalidateAllEntitlements(db, deps, revalidate?)` (one isolated pass — per-org try/catch, returns `{total, ok, failed}`, `revalidate` injectable for testing, defaults to the existing `revalidateLicense` whose offline-grace semantics are **unchanged** — the orchestrator only loops it); `resolveIntervalMs` + `DEFAULT_INTERVAL_MS = 43_200_000` (**12h**, env-configurable via `SCHEDULER_INTERVAL_MS`); the thin `bin/scheduler.ts` loop (pass-on-start then every interval, SIGTERM/SIGINT stop, no vitest test — mirrors `apps/worker/src/index.ts`). Packaging: a `scheduler` compose service reusing the Plan-6a `tsx-app` target (`APP=control-plane` + command override, **no Dockerfile change**), listing all three env vars explicitly (`DATABASE_URL`/`LICENSE_KEY_SECRET`/`SCHEDULER_INTERVAL_MS` — the Plan-6a interpolation-only Critical does **not** recur), gated on `postgres: healthy` + `migrate: completed`; `SCHEDULER_INTERVAL_MS` documented in `.env.example` (env-drift guard green). Verified: `tsc -b` clean; control-plane **167→174 pass / 3 skip**, root **192 / 4** unchanged; a full-stack `docker compose up --build` ran the scheduler live (`revalidation pass: 0/0 ok, 0 failed` at the 12h default). Whole-branch review: **no Critical, no Important** — ship as-is. **Every pass revalidates all entitled orgs (no due-window filter in v1)** — a stated scale deferral, unnecessary for a single-operator instance.

- **6c deferred Minors (reviewer-triaged, non-blocking — accepted as-is):** (1) **`bin/scheduler.ts` sleep is not interruptible** — a SIGTERM arriving mid-sleep can't wake the timer, so at the 12h default `docker stop`'s ~10s grace elapses and the container is SIGKILLed (exit 137) on *every* shutdown. Harmless (the loop is idle when sleeping, `revalidateLicense` is idempotent/background-safe, no partial write is in flight), but a fix (resolve the sleep promise on the stop handler) would **edit plan-verbatim code the brief scoped the stop-check for — the plan owner's call**, not a unilateral fix. (2) **No `restart:` policy on `scheduler` (nor on `worker`)** — with Compose's default `restart: "no"`, if either loop ever exits it stays down until noticed; *consistent with the existing `worker` convention, not a regression* — `restart: unless-stopped` on both would make the deployment self-healing (out of 6c scope). (3) **`resolveIntervalMs` has no lower bound** — `SCHEDULER_INTERVAL_MS=1` is accepted and would poll Lemon Squeezy every 1ms; operator-controlled and low-risk, but a floor (clamp to ≥60_000) would be defensive.
