...
- **Plan 5 (ConfigStore caching):** deliberately NOT added in Plan 4 — a cache without invalidation serves stale config after an operator edit. Add a `CachingConfigStore` in Plan 5 paired with the Redis pub/sub invalidation the control plane publishes on config writes (per the Plan 2 carry-forward). Document the pairing.
...
- **Plan 5.4 (do this FIRST, before Keys):** **hoist the fence config Zod schemas** (`rateLimitSchema`, `quotaRuleSchema`, `quotaSchema`) into `@metamodels/schema` (already the shared home of `METER_DIMS`) and import them from BOTH `apps/control-plane/src/lib/fence-schema.ts` AND `apps/data-plane/src/config/quota.ts`. Today they are hand-copied and identical, but a later `period` value / rate-limit field added on one side would silently break the other's enforcement. Cheap now, expensive later. (Whole-branch Minor #1 — plan-acknowledged intentional duplication.)
- **Plan 5.4 (Keys):** the paddock→flock org-consistency built in 5.2 is the **key↔paddock groundwork**; 5.4 adds the key↔paddock link (a key may only be scoped to paddocks in its org) using the same org-scoped-join pattern (`paddockBreedInOrg` in `fences-service.ts` is the reference).
- **Plan 5.3 (paramSchema editor):** ComfyUI fence **templates** are authored here. 5.2's fence editor round-trips the existing `{templates:[…]}` through a hidden field when an operator edits rate/quota on a ComfyUI paddock (safe — no data loss). 5.3 replaces that with real template authoring. Note the full workflow-graph JSON currently ships in the 9c page HTML (bloaty for large graphs) — 5.3's editor should manage templates server-side rather than round-tripping them through the client.
