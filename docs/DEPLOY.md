...
| `UPSTREAM_AUTH_KEY` | migrate, control-plane, data-plane | Base64 of **exactly** 32 random bytes: `openssl rand -base64 32`. Encrypts each flock's upstream credential at rest; all three services must hold the same value, and none starts without it. Rotate it through `UPSTREAM_AUTH_PREVIOUS_KEYS` — see [Rotating the upstream credential key](#rotating-the-upstream-credential-key). The `.env.example` value is for local use and CI only. |
